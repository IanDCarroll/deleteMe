...
describe 'IanStrscan scan' do
  context 'when regex matches the current postion' do
    Given(:string) { 'abc def' }
    When(:subject) { IanStrscan.new(string) }
    Then { 'abc' == subject.scan(/\w+/) }
  end

  context 'when regex does not match the current postion' do
    Given(:string) { 'abc def' }
    When(:subject) { IanStrscan.new(string) }
    Then { nil == subject.scan(/\s+/) }
  end

  context 'when scan remembers its place in the string' do
    Given(:string) { 'abc def' }
    When(:subject) { IanStrscan.new(string) }
    Then { 'abc' == subject.scan(/\w+/) }
    And  { ' ' == subject.scan(/\s+/) }
  ...
  context 'when scan does not eat the original string' do
    Given(:string) { 'abc def' }
    When(:subject) { IanStrscan.new(string) }
    Then { 'abc' == subject.scan(/\w+/) }
    And  { 'abc def' == string }
